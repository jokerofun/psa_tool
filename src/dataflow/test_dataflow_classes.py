...
class TestDataflowNodes(unittest.TestCase):
    ...
    def test_data_fetching_from_api(self, mock_requests_get):
        ...
        node = DataFetchingFromAPINode(
            name="apiPrices", source="http://fakeapi.com/data")
        node.run()
        results = node.get_results()
    ...
    def test_data_processing_node(self):
        ...
        with patch("pandas.read_csv") as mock_read_csv:
            ...
            file_node = DataFetchingFromFileNode(
                name="input", source="fake.csv")
            process_node = DataProcessingNode(
                name="processor", process_func=sample_processing)
            ...
            self.assertTrue((results["processed"]["col1"] == [2, 4]).all())
    ...
    def test_dependency_execution_order(self, mock_run):
        ...
        pricesEUR = DataFetchingFromFileNode(
            name="pricesEUR", source="fake.csv")
        processNode = DataProcessingNode(
            name="process", process_func=lambda x: x)
